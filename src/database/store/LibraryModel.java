...
public class LibraryModel{
	...
	public void searchPlayListName(String name) {
		...
		if(pList.isEmpty()) {
			...
			for(PlayList p: pList) { // Prints the playlists retrieved from the resulted iteration
				System.out.println(p.toString());
			}
		}
	}
...
///////////////////////////////////////////////////////////////////////////////////////////////	
	...
	public void playSong() {
	    ...
	    if(artists.size() > 1) {
	    	...
	    	String userAuthor = scan.nextLine().toLowerCase().trim();
	    	...
	    	for(Song sg : getSongLibrary()) {
	    		if(sg.getTitle().toLowerCase().equals(sT)) { 
	    			System.out.println("Playing " + sg.getTitle() + ".....Enjoy!");
	    			queList.addQue(sg);
	    		}
	    	}
	    }
	    if(!checkUserList("Re")) {
	    	makePlayList("Fr");
	    	makePlayList("Re");
		    for(Song r : queList.getQueue()) { addSongToPlayList("Re", r.getTitle()); }
		    for(Song f : queList.retrieveListOfFreq()) { addSongToPlayList("Fr", f.getTitle()); }
	    } else {
		    for(Song r : queList.getQueue()) { addSongToPlayList("Re", r.getTitle()); }
		    for(Song f : queList.retrieveListOfFreq()) { addSongToPlayList("Fr", f.getTitle()); }
	    }
	...
	//Methods that print out the QueueList
	public void getRecentlyPlayed(){
		System.out.println(queList.getRecentlyPlayed());
	}
...
///////////////////////////////////////////////////////////////////////////////////////////////	
	...
	//checks if a playlist is inside the library
	/*
	 * @pre: compare != null
	 */
	private boolean checkUserList(String p_name) {
		for(PlayList p : userList) {
			if(p.getTitle().equalsIgnoreCase(p_name)) {
				return true;
			}
		}
		return false;
